...
# rephraseGraph(connectivity = 'full')
...
def graphofroom(**kwargs):
    ...
    # DEEP DEFINITION:
    # La idea es definir el grafo con un mínimo tamaño de malla. Si la medida en metros tiene
    # cierta precisión, debe bajarse a ese nivel.
    # Ej: 2.75 mts -> 275 unidades mínimo
    # Por lo tanto el primer paso es realizar una conversión del mayor grado de precisión
    # entre todos los datos de entrada

    # for decimal check, must be a string
...
def boxesinroom(**kwargs):
    ...
    path = kwargs['path']
    nodes = kwargs['nodes']
    # PATH is composed of a set of lines. Each line is:
    #   initialpoint = (x_1, y_1, z_1)
    #   end_point = (x_2, y_2, z_2)
    # Esto crea una caja que restringe las conexiones entre
    # los puntos dentro de ese espacio.
    ...
    with open(path) as restrictions:
        for line in restrictions:
            ...
            #box_points = [point for point in itertools.product(x_range, y_range, z_range)]
